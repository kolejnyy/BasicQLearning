...
from ToxicToads import isOver, gameScore, startState, randomState, move, printState, possibleMoves
...
class QNet(nn.Module):
	...
	def __init__(self):
		...
		# Layers
		self.linr1 = nn.Linear(25, 128)
		self.linr2 = nn.Linear(128, 128)
		self.linr3 = nn.Linear(128, 128)
		self.linr4 = nn.Linear(128, 25)
...
def playout(state, player, exp_rate, debug=False):
	...
	possible_actions = possibleMoves(state, 1)
	...
	if debug:
		printState(state)
		print(np.array(values(state)).reshape(5,5))
